...
void insert_line(player *p,char *str)
{
  ...
  to=from+len;
  while(from!=e->current) *(--to)=*(--from);
  *(--to)=*(--from);
  e->size+=len;
  for(;len;len--) *(e->current)++=*stack++;
  *stack++=0;
  tell_player(p,oldstack);
  stack=oldstack;
}
...
void finish_edit(player *p)
{
  if (!(p->edit_info)) return;
  restore_flags(p);
  if (p->edit_info->buffer) FREE(p->edit_info->buffer);
  FREE(p->edit_info);
  p->edit_info=0;
  p->flags &= ~DONT_CHECK_PIPE;
}
...
void back_page(player *p,ed_info *e)
{
  ...
  int n;
  scan=e->current;
  for(n=TERM_LINES-2;n;n--) {
    while(scan!=e->buffer && *scan!='\n') scan--;
    if (scan==e->buffer) break;
    e->size--;
    scan--;
  }
  e->current=scan;
